...
import { DROPLET_FILE_PREFIX, listFiles } from "../src/lib";
import { TESTS_DROPLET_DIRECTORY_PATH } from "./_tests";

describe("listFiles", () =>
  it(
    "should list all files prefixed with " +
      DROPLET_FILE_PREFIX +
      " in a path",
    () => {
      expect(listFiles(TESTS_DROPLET_DIRECTORY_PATH)).toEqual([
        ...
        },
        {
          ...
          trimmed:
            "/droplet_directory_nested/droplet_directory_nested/droplet_template_template/droplet_file_{{droplet}}.ts.hbs",
          trimmedCleaned:
            "/nested/nested/template/{{droplet}}.ts"
        },
        {
        ...
        },
        {
          ...
          trimmed:
            "/droplet_directory_nested/droplet_template_template/droplet_file_exampleWithDroplet.ts.hbs",
          trimmedCleaned:
            "/nested/template/exampleWithDroplet.ts"
        },
        {
          cleaned: "{{droplet}}.ts",
          original: join(
        ...
        },
        {
          ...
          trimmed:
            "/droplet_template_{{droplet}}/droplet_file_{{droplet}}WithDroplet.ts.hbs",
          trimmedCleaned:
            "/{{droplet}}/{{droplet}}WithDroplet.ts"
        }
      ]);
    }
  ));
