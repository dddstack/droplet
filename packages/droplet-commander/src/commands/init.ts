...
const PUBLIC_INIT_DIRECTORY_PATH = join(dirname(fileURLToPath(import.meta.url)), "public", "init");

export const init = (options: Options) => {
  const dropletDirectory = join(options.fromDirectory, DROPLET_DIRECTORY_NAME);

  log({ command: "INIT", message: "Looking for Droplet...", status: 1, statusOf: 3, type: "status" });

  if (existsSync(dropletDirectory)) {
    log({ message: `Droplet already exists at ${dropletDirectory}`, type: "error" });
    exit(0);
  }
